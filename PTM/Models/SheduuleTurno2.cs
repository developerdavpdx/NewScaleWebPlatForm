...
{
    public class SheduleTurno2: Registry
    {
        public SheduleTurno2()
        {
        ...
        public void ExecuteJobTurno2()
        {
            try // Código de la primera tarea programada (turno 1)
            {
                ...
                DateTime horaFinTurno2 = DateTime.Today.AddHours(4.5); // Fecha y hora de fin del turno 2 (mañana a las 4:30 am) // Fecha y hora de fin del turno 1 (hoy a las 4:30 pm)


                sendReport(2, "PPVC", horaInicioTurno2, horaFinTurno2);
                sendReport(2, "INY", horaInicioTurno2, horaFinTurno2);
                
                string rutaVirtualE = "~/Shedule/";
                string rutaEmailE = HostingEnvironment.MapPath(rutaVirtualE);
                if (!System.IO.File.Exists(rutaEmailE + Path.GetFileName("CorreoEnvicado.txt")))
                {
                    System.IO.File.Create(rutaEmailE + Path.GetFileName("CorreoEnvicado.txt")).Close();
                }

                using (StreamWriter writer = File.AppendText(rutaEmailE + Path.GetFileName("CorreoEnvicado.txt")))
                {
                    writer.Write($"{DateTime.Now} - Ejecuta Job 2");
                    writer.Close();
                }
            }
            catch (Exception ex)
            {
                ...
                //string rutaEmail = Directory.GetCurrentDirectory() + "/Shedule/ErroresMail.txt";
                string rutaVirtualE = "~/Shedule/";
                string rutaEmailE = HostingEnvironment.MapPath(rutaVirtualE);
        ...
        public void sendReport(int turno, string proceso, DateTime fechaTurnoInicio, DateTime fechaTurnoFin)
        {
            try
            {
                ...
                catch (Exception ex)
                {
                    ...
                    string rutaVirtualE = "~/Shedule/";
                    string rutaEmailE = HostingEnvironment.MapPath(rutaVirtualE + Path.GetFileName("ErrorSend.txt"));
            ...
            catch (Exception ex)
            {
                ...
                string rutaVirtualE = "~/Shedule/";
                string rutaEmailE = HostingEnvironment.MapPath(rutaVirtualE + Path.GetFileName("ErroresEnvioMail.txt"));
